; ClassManager.ClassTypes.Class = (function() {
     ...
     * @constructor
     */
    var Class = ClassManager.ClassTypes.Class = function Class(classManager, className, classDefinition)
    {
    ...
    Class.$parent = ClassManager.ClassTypes.ClassType;
    ...
    {
        return ClassManager.ClassTypes.Class.Builder;
    };
    ...
    Class.prototype.setClassParent = function (parentClassName)
    {
        ...
        if (
            ...
            && this._classParent.constructor != ClassManager.getClassType('AbstractClass')
        ) {
    ...
    {
        if (ClassManager.issetClassType("Trait") && this.getTraits().length) {
            ...
            for (var i = 0; i < traits.length; i++) {
                ...
                ClassManager.Tools.extend(this._classProperties, traitProperties);
            }
        }
    ...
    Class.prototype.createClassConstructor = function ()
    {
        ...
        if (
            !ClassManager.issetClassType('AbstractClass')
            || (
                ClassManager.issetClassType('AbstractClass')
                && this.constructor != ClassManager.ClassTypes.AbstractClass
            )
        ) {
    ...
    Class.prototype.getBaseClassDefinition = function ()
    {
        ...
        if (ClassManager.issetClassType('Trait')) {
        ...
        if (ClassManager.issetClassType('Interface')) {
    ...
    {
        ClassManager.Tools.extend(this._abstractMethods, methods);
    };
    ...
    Class.prototype.addTrait = function (traitName)
    {
        ...
        if (traitClass.constructor != ClassManager.ClassTypes.Trait) {
        ...
        this._classDefinition = ClassManager.Tools.extend(traitProps, classDefinition);
    ...
    Class.prototype.addInterface = function (interfaceName)
    {
        ...
        var interfaceClassConstructorProto = ClassManager.Tools.copy(interfaceClassConstructor.prototype);
        var interfaceClassProperties = interfaceClass.getClassDefinitionProperties();
        var abstractMethods = {};

        if (interfaceClass.constructor != ClassManager.ClassTypes.Interface) {
        ...
        loop: for (var methodName in interfaceClassConstructorProto) {
            ...
            for (propName in interfaceClassProperties) {
                ...
                var setterName = ClassManager.Tools.generateSetterName(propName);
                var getterName = ClassManager.Tools.generateGetterName(propName);
    ...
    ClassManager.registerClassType(Class);
