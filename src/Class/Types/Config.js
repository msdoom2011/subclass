; ClassManager.ClassTypes.Config = (function()
{
    /*************************************************/
    ...
    Config.$parent = ClassManager.ClassTypes.ClassType;
    ...
    {
        return ClassManager.ClassTypes.Config.Builder;
    };
    ...
    Config.prototype.normalizeClassProperties = function()
    {
        ...
        for (var i = 0, includes = this.getIncludes(); i < includes.length; i++) {
            ...
            for (propName in includeClassProperties) {
                ...
                if (!classProperties[propName]) {
                    classProperties[propName] = ClassManager.Tools.copy(includeClassProperties[propName]);
                }
            }
        }
        ...
        for (propName in classProperties) {
            ...
            if (!property || !ClassManager.Tools.isPlainObject(property)) {
    ...
    Config.prototype.extendClassProperties = function(childProperties, parentProperties)
    {
        for (var propName in childProperties) {
            ...
            if (
                ClassManager.Tools.isPlainObject(childProperties[propName])
                && parentProperties[propName]
            ) {
                childProperties[propName] = ClassManager.Tools.extendDeep(
                    ClassManager.Tools.copy(parentProperties[propName]),
    ...
    ClassManager.registerClassType(Config);
